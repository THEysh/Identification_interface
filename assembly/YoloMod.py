...
class YoloModel:
    def __init__(self):
        # 检查路径是否存在
        if ('modelPath' in ConfGlobals) and os.path.exists(ConfGlobals['modelPath']):
            self.yolo = YOLO(ConfGlobals['modelPath'], task="detect")
            print("ini 模型路径存在， 加载成功")
        elif os.path.exists('./resource/best.pt'):
