...
class InferenceThread(QThread):
    ...
    def __init__(self, model, data, thread_id):
        super().__init__()
        self.model = model
        self.data = data
    ...
    def run(self):
        try:
            ...
            # 如果出现异常，发出错误信号
            self.error_signal.emit(f"Thread {self.thread_id}: {str(e)}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 创建 YoloModel 实例
        self.model1 = YoloModel()
        self.model2 = YoloModel()
    ...
    def on_inference_result(self, result):
        # 处理推理结果并更新 UI
        thread_id, save_dir, rectangle_pos, scores, classes, imgshape, orgimgpath, runtime = result
        result_text = f"线程 {thread_id}:\n" \
                      f"结果路径: {save_dir}\n" \
                      ...
                      f"原图路径: {orgimgpath}\n" \
                      f"运行时间: {runtime}秒\n"
        self.label.setText(self.label.text() + result_text)

    def on_error(self, error_message):
        # 处理错误并更新 UI
        self.label.setText(self.label.text() + f"错误: {error_message}\n")
